...
usersController.register = async (req, res, next) => {
    try {
        ...
        console.log("phone", phonenumber, password, username);
...
usersController.show = async (req, res, next) => {
    try {
        ...
        let user = await UserModel.findById(userId).select('phonenumber username gender birthday avatar cover_image blocked_inbox blocked_diary address description').populate('avatar').populate('cover_image');
        if (user == null) {
            return res.status(httpStatus.NOT_FOUND).json({message: "Can not find user"});
        }
