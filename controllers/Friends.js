...
friendsController.getRequested = async (req, res, next) => {
    try {
        let sender = req.userId;
        let receiverd = await FriendModel.find({sender: sender, status: "0" }).distinct('receiver');
        let users = await UserModel.find().where('_id').in(receiverd).populate('avatar').populate('cover_image').exec()
   
        res.status(200).json({
            code: 200,
            message: "Danh sách lời mời kết bạn mà bạn gửi",
            data: {
                friends: users,
            }
        });
